...
class TestChatEndpoint:
    ...
    def test_sse_token_with_newline_uses_json_framing(self, monkeypatch):
        ...
        assert 'data: {"text": "line two"}' in response.text
    ...
    def test_websocket_forwards_first_safe_sentence_before_generation_finishes(self):
        ...
        async def scenario():
            ...
            stream_task = asyncio.create_task(
                _stream_llm_to_websocket(
                    websocket,
                    SlowLLM(),
                    "prompt",
                    result_state={"intent": "chat", "_execution": []},
                )
            )
            try:
