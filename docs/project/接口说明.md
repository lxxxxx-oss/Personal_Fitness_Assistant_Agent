...
```text
event: meta
data: {"conversation_id":"conv_xxx","model":"qwen-local","intent":"diet","sources":[],"warnings":[],"execution":[{"component":"rag","mode":"sqlite_faiss","degraded":false,"detail":""},{"component":"llm","mode":"local_qwen","degraded":false,"detail":""}]}
...
| 事件 | 说明 |
|---|---|
| `meta` | 首个事件，返回 conversation_id、model、intent、sources、warnings 和 execution 执行轨迹 |
...
- 用户在聊天中明确说“请记住/帮我记住/记住……”时会走显式 Memory Writer；少量带有“长期/以后/平时”等稳定标记的隐式表达只生成候选，不会自动写入正式记忆。
- 普通显式记忆会写入 `memory_items`；健康敏感、同槽位冲突和隐式长期内容先进入 `candidate_memories`，确认后才生效；口令、Token 等秘密信息不持久化。
