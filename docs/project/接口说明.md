...
```text
event: meta
data: {"conversation_id":"conv_xxx","intent":"diet","sources":[],"warnings":[],"execution":[{"component":"rag","mode":"sqlite_faiss","degraded":false,"detail":""},{"component":"llm","mode":"local_qwen","degraded":false,"detail":""}]}
...
实现说明：HTTP、SSE、WebSocket 共用会话解析、记忆恢复、RouterState 构建和成功 turn 持久化流程。LangGraph 的同步准备阶段通过工作线程执行；本地 LLM 的同步 token 生成器也通过线程到 asyncio queue 桥接，再由 SSE async iterator 逐 token 输出。这样不会阻塞 FastAPI 事件循环，但不会提高模型本身的并行吞吐，单进程生成仍受共享模型锁串行控制。
...
- WebSocket 与 HTTP/SSE 复用同一输入约束：`user_id` 为 1-64 字符串，`message` 为 1-4096 字符串，`conversation_id` 可选；非法 JSON 返回 `INVALID_JSON`，字段非法返回 `INVALID_REQUEST`。
...
| 503 | Motion 可选依赖或 MediaPipe 模型资产缺失 |
| 500 | 未分类的服务内部错误；响应只返回稳定错误码与安全消息，不暴露原始异常 |
| 503 | 本地语言模型或必要运行依赖不可用 |
