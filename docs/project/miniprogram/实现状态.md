...
本页是小程序唯一的当前状态和验收入口。详细架构看[设计说明](./设计说明.md)。

## 结论

小程序聊天端和 Motion 图片/视频上传的代码级闭环已经完成。用户通过统一媒体入口选择相册或相机，图片返回单帧姿态摘要；视频展示本地播放器、上传进度、多帧 PoseSequence 指标和真实执行标签。后端 WebSocket 保留 `token` 事件协议，但会先缓冲到句子或换行边界并完成输出安全校验，再发送句段级增量。由于微信开发者工具和真机端到端联调尚未完成，当前不能把它表述成“移动端已经完整验收上线”。

| 模块 | 状态 | 说明 |
|---|---|---|
| 项目脚手架 | 已完成 | 原生小程序结构、配置、主题和本地存储 |
| API 封装 | 已完成 | health、chat、WebSocket、history、clear、Motion 图片/视频上传 |
| 流式接收 | 已完成 | WebSocket meta/token/done、后端安全句段增量转发、端侧增量更新和 HTTP 降级 |
...
2. 验证五类 intent、token 增量渲染、sources、warnings、execution、历史记录和空状态。
3. 验证 WebSocket 建连或执行失败后能降级 HTTP，且不会生成重复消息。
4. 验证图片/视频选择与预览、10MB/30MB 限制、上传进度和 413/422/503 错误提示。
5. 验证有效帧、抽样帧、FPS、置信度、warning 和相似度结果；无兼容参考时只能返回姿态提取，不能伪造相似度。
6. 真机配置 HTTPS/WSS 以及 `request`、`socket`、`uploadFile` 合法域名，检查隐私授权。
7. 真机验证弱网、断网、后台切换、大文件上传和长回复滚动性能。

只有开发者工具和真机核心链路都通过，才能将“代码级闭环已完成”升级为“移动端已完成端到端验收”。在此之前，面试中统一说明：代码链路已打通，开发者工具、真机、HTTPS 和弱网仍待完整验收。
