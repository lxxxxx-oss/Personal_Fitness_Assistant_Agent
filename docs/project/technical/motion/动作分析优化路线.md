...
| 端侧媒体上传体验 | 部分实现 | Web UI 和小程序均可独立上传并展示分析结果；Web UI 尚未把制品自动带入聊天请求，小程序真机待验收 |
...
- 当前停点：前端附件联动尚未实现，用户界面还不会自动上传并把制品 ID 带入 HTTP/SSE/WS 对话请求。
- 下一步只有在确认后才进入：实现 Web UI 的媒体选择、上传状态、制品引用和失败提示。
...
当前状态：独立上传和结果展示已实现；小程序的微信开发者工具与真机仍待验收，Web UI 的“上传后自动带入 ChatRequest”尚未实现。
...
- Web UI 当前直接调用图片/视频分析接口并渲染独立结果卡，尚未保存 `artifact_id` 并通过 `motion_artifact_ids` 交给 Router。
...
步骤 1—5 的核心工具链、步骤 6 的局部解释指标和步骤 8 的独立上传体验已完成。按当前确认制实施顺序，先补齐 Web UI 与 Chat 的自动串联，再继续动作质量闭环：

1. Web UI 上传时传入逻辑用户和会话信息，保存返回的 `artifact_id`，并随同一次聊天请求发送 `motion_artifact_ids`。
2. 为上传失败、制品过期和对话失败补充可重试状态及静态契约/联合回归测试。
3. 对视频关键点做基础平滑和缺失帧处理，并增加单次深蹲动作周期切分。
4. 采集与 `mediapipe_33` 一致、同视角并经教练确认的正式标准动作样本。
5. 为深蹲等动作增加幅度、对称性、阶段级规则和典型错误标签。
