...
> 归档说明：本文是整理前的子图优化长文档，保留用于追溯历史路线和设计细节。当前面试复习请优先使用 `docs/learning/`；专题设计看 `docs/project/technical/router/`、`docs/project/technical/motion/`，当前验证结果统一看 `docs/project/项目证据.md`。
...
- 后续 Router 和 Motion 的当前状态分别维护在 `docs/project/technical/router/`、`docs/project/technical/motion/`；其他模块的当前事实以 `docs/project/项目总览.md`、最新 progress/tests 和代码为准。
...
Motion 的核心优化方向不是让用户直接准备 `.npz`，而是把当前已实现的“姿态序列分析后半段”扩展为普通用户可使用的图片/视频动作分析。详细需求和技术路线见 [动作媒体链路设计.md](../motion/动作媒体链路设计.md)，持续维护台账见 [动作分析优化路线.md](../motion/动作分析优化路线.md)。
...
以下是摘要版路线；每一步的当前状态、实际做法和路线偏差以后统一维护在 [动作分析优化路线.md](../motion/动作分析优化路线.md)。
...
详细设计见 [多意图路由设计.md](../router/多意图路由设计.md)。
