...
> 这是早期任务拆解，复选框不再作为当前进度依据。文中 SSE + `enableChunked` 是早期路线；当前代码已经改为 WebSocket 主路径 + HTTP 降级。实际完成度和现行协议统一查看 [实现状态.md](./实现状态.md) 与 [设计说明.md](./设计说明.md)。

本计划中的 `mcp` 菜谱页面和验收项对应当前代码。产品层可以与 Diet 共用“饮食与菜谱”入口，但 intent 标签继续反映实际执行路径。

> **关联文档:** [小程序设计规范](./设计说明.md) | [后端 API 文档](../接口说明.md) | [后端实施计划](../superpowers/plans/2026-06-09_健身助手实施计划.md)
