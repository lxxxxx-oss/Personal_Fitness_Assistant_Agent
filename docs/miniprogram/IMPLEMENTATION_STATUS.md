...
小程序聊天端代码已经完成，支持五类 intent 展示、WebSocket 流式接收、HTTP 非流式降级、来源与运行提示卡片，并用执行标签明确区分真实依赖和 mock/fallback；当前主要缺口是媒体上传、开发者工具/真机端到端联调与体验优化。
...
2. 验证五类 intent 的普通与 WebSocket 流式响应，以及 sources/warnings/execution 展示。
