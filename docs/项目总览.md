...
本文档是项目当前代码状态的事实入口：回答已经做到哪里、当前边界和下一步。项目的首要目标是服务 Agent 岗位面试，健身是承载技术设计的业务场景；零基础学习请先看 [README.md](./README.md) 和 [零基础项目地图](./interview/01_零基础项目地图.md)。运行命令集中在 [运行与排错.md](./运行与排错.md)，接口细节集中在 [接口说明.md](./接口说明.md)。

## 先看哪份

如果你只是为了面试复习，不需要从本文档从头读到尾，直接按这个顺序：

1. [README.md](./README.md)：完整学习顺序。
2. [interview/01_零基础项目地图.md](./interview/01_零基础项目地图.md)：先听懂项目和 Agent。
3. [interview/02_项目讲解与面试话术.md](./interview/02_项目讲解与面试话术.md)：再练项目介绍。
4. [interview/04_高频技术问答.md](./interview/04_高频技术问答.md)：最后准备技术追问。
...
详细请求字段、响应结构、状态码和示例见 [接口说明.md](./接口说明.md)。
...
Motion 图片/视频分析需要额外安装 `requirements-motion.txt` 并准备 `data/models/pose_landmarker.task`。完整安装、配置、测试、Router eval、Docker 和小程序联调命令见 [运行与排错.md](./运行与排错.md)。
...
2. 按 [Motion 优化路线](./technical/motion/动作分析优化路线.md) 扩充标准动作样本库、关键点平滑、动作周期切分和专项纠错规则。
...
| 想了解什么 | 阅读入口 |
|---|---|
| 面试学习从哪里开始 | [README.md](./README.md) |
| 项目当前状态 | 本文档 |
| 接口协议 | [接口说明.md](./接口说明.md) |
| 运行和联调 | [运行与排错.md](./运行与排错.md) |
| 面试复习 | [面试资料入口](./interview/README.md) |
| 技术设计 | [技术细节入口](./technical/README.md) |
| 优化设计 | [优化路线入口](./optimization/README.md) |
| 项目证据 | [项目证据.md](./项目证据.md) |

当前事实以 `项目总览.md` 和 `接口说明.md` 为准；历史计划和 progress 记录不能覆盖已经更新的当前状态。
