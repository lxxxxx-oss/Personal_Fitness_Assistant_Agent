...
检索只返回 `status='active'` 的正式长期记忆，不返回 pending/rejected candidate。返回项包含 `score` 字段，当前分数由关键词召回、可选 embedding、importance 和访问次数加权得到；未开启向量增强时 embedding 项为 0。访问次数加成采用 `0.10 * min(access_count, 20) / 20`，避免访问次数无限放大。
