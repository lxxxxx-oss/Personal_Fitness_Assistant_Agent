...
本文档是项目当前代码状态的事实入口：回答已经做到哪里、当前边界和下一步。项目的首要目标是服务 Agent 岗位面试，健身是承载技术设计的业务场景；零基础学习请先看 [00_先看这里.md](./00_先看这里.md) 和 [零基础项目地图](./interview/00_零基础项目地图.md)。运行命令集中在 [03_运行排错.md](./03_运行排错.md)，接口细节集中在 [02_接口说明.md](./02_接口说明.md)。

## 先看哪份

如果你只是为了面试复习，不需要从本文档从头读到尾，直接按这个顺序：

1. [00_先看这里.md](./00_先看这里.md)：完整学习顺序。
2. [interview/00_零基础项目地图.md](./interview/00_零基础项目地图.md)：先听懂项目和 Agent。
3. [interview/01_MUST_MASTER_PROJECT_STORY.md](./interview/01_MUST_MASTER_PROJECT_STORY.md)：再练项目介绍。
4. [interview/02_SHOULD_MASTER_TECH_QA.md](./interview/02_SHOULD_MASTER_TECH_QA.md)：最后准备技术追问。

下面内容是“项目事实说明”，用于查状态和边界，不要求背诵。
...
| Tool System | 已落地最小 `ToolRegistry` 原型：`ToolSpec` 记录 name、description、input_schema、permission、executor、timeout_seconds 策略字段、retry、fallback；Registry 支持注册、列出、schema 校验、权限检查、执行、有限重试、fallback、`execution_id`、`duration_ms` 和 audit log；Search、Knowledge/RAG 与 MCP execute 已接入 Registry，`motion.compare_pose` 作为代表工具已注册但当前 Motion 媒体链路仍由 LangGraph/API 直接控制 |
...
当前自动化测试结果为 `189 passed, 2 skipped, 2 warnings`，复现环境为 `D:\Users\Lesedi\anaconda3\envs\fitness-agent\python.exe -m pytest tests/ -q`。默认 pytest 通过 fixture 替换本地 LLM 生成和 SentenceTransformer 编码，主要证明接口、状态流、工具治理、算法与降级契约可回归；两个 skip 分别是本地真实模型和需显式 `MILVUS_TEST_URI` 的真实 Milvus 测试。最新 Memory + Context + RAG 专项 benchmark 结果为 8/8 passed，相关核心回归集为 `137 passed, 1 warning`。真实 Qwen Router A/B、MediaPipe 媒体冒烟另有专项记录。两个 warning 分别来自 Starlette TestClient/httpx 兼容层弃用提示，以及当前 Windows 工作区 `.pytest_cache` 写入受限提示。关键证据见 [04_项目证据.md](./04_项目证据.md)。
...
| 工具系统容易被问是否只是函数调用 | 已补最小 `ToolSpec + ToolRegistry` 原型，统一管理 schema、权限、超时策略字段、有限重试、fallback、`execution_id`、`duration_ms` 和审计；Search、Knowledge/RAG 与 MCP execute 已接入；`motion.compare_pose` 已作为代表工具注册，但 Motion 媒体链路没有全面迁移 | 后续如继续工具治理，再评估 Motion 标准动作比较是否真正接入主执行路径；媒体上传和姿态估计继续保留 API 边界；Registry 层硬超时中断仍需补齐 |
...
详细请求字段、响应结构、状态码和示例见 [02_接口说明.md](./02_接口说明.md)。
...
Motion 图片/视频分析需要额外安装 `requirements-motion.txt` 并准备 `data/models/pose_landmarker.task`。完整安装、配置、测试、Router eval、Docker 和小程序联调命令见 [03_运行排错.md](./03_运行排错.md)。
...
docs/04_项目证据.md             面试高价值进度与测试证据摘要
...
3. ToolRegistry 阶段已先收束，Search、Knowledge/RAG 与 MCP execute 已接入；后续如继续工具治理，再评估 Motion 标准动作比较是否接入 `ToolRegistry`。
...
| 想了解什么 | 阅读入口 |
|---|---|
| 面试学习从哪里开始 | [00_先看这里.md](./00_先看这里.md) |
| 项目当前状态 | 本文档 |
| 接口协议 | [02_接口说明.md](./02_接口说明.md) |
| 运行和联调 | [03_运行排错.md](./03_运行排错.md) |
| 面试复习 | [面试资料入口](./interview/README.md) |
| 技术设计 | [技术细节入口](./technical/README.md) |
| 优化设计 | [优化路线入口](./optimization/README.md) |
| 项目证据 | [04_项目证据.md](./04_项目证据.md) |

当前事实以 `01_项目总览.md` 和 `02_接口说明.md` 为准；历史计划和 progress 记录不能覆盖已经更新的当前状态。
