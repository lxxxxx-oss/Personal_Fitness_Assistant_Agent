...
- 已完成：简化版持久化摘要的存储基础，包含完整 turn 事务写入、active summary 替换、消息边界和用户/会话隔离。
- 已完成：确定性抽取式摘要器，包含字符阈值、摘要长度限制、上一版摘要复用和最近三轮保留。
- 已完成：HTTP、SSE、WebSocket 统一接入，Chat/Diet/Search/MCP 最终 Prompt 注入和摘要失败 fallback。
...
- 已完成：上下文阶段 4 的确定性 prompt compact，以及阶段 5 的后端 `execution/_prompt_meta` 可观测信息；Web/小程序“对话已浓缩”状态卡尚未实现。
- 已完成第一版：记忆阶段 6 的 embedding jobs、本地处理接口和可选 Milvus 用户记忆派生索引；当前不是常驻后台 worker，也没有证明语义召回一定优于 SQLite 基线。
- 已完成：联动与评测，新增 Memory + Context + RAG 统一 benchmark，覆盖长期记忆召回、敏感 candidate、prompt 注入、compact 和 RAG source 透传。
- 下一步候选：扩大多轮摘要样本，验证目标修改、无关信息和更长会话；模型摘要与精确 token 只作为生产化方向。
...
- 当前只处理用户明确“记住……”的消息；从 `_structured_state.profile / user_context / decisions` 自动提升长期事实仍是后续候选抽取方向
...
- 与已有记忆的语义冲突识别和 relation 管理仍是后续方向；当前只有精确 `memory_key` 去重
...
- 实现：
  ...
  - final_score
  - 分层预算注入
- 当前按检索分数和字符预算注入；pinned rule / constraint 是生产化目标字段，尚未实现
...
- 建表：`embedding_jobs`
- 当前原型使用本地处理接口 `/memory/embedding-jobs/process`：
  - 读取 pending jobs
  - Sentence-Transformers 编码
  - Milvus upsert
  - 成功后标记 completed
- 生产化再替换为常驻后台 worker 或任务队列
- 失败时指数退避重试，最多 5 次
- Milvus 不可用时降级到 FTS5 + metadata

验收标准：

- Milvus 可用时语义召回链路能够执行；是否提升质量必须由扩大后的 memory golden set 证明。
- Milvus 不可用时主流程不挂。
...
- 建 memory eval case：
  ...
  - Recall@K
  - 冲突识别率
- 后续增加 `memory_access_logs` 清理策略；当前仅保留轻量 `access_count` / `last_accessed_at`
- 生产化补：
  - 登录鉴权
  - 用户授权
  - 加密/脱敏
...
- 当前 benchmark 已达到 14/14 passed，其中会话摘要 6/6；全量回归结果统一以[项目证据](../项目证据.md)为准，避免在实施计划中保留过期数字。
...
- 超阈值时：
  ...
  - 旧轮次提取 `user_context`
  - 各字段按上限裁剪
- 当前使用确定性字符 compact，不调用 LLM 摘要；压缩异常时不能破坏主流程
...
面试时不要把生产化治理说成已经完整实现，应按下面方式表达：

> 当前项目已经从最初的滑动窗口升级为分层记忆：SQLite 保存完整会话和长期记忆，SlidingWindowMemory 只做近期 hot cache；较早会话通过确定性 active summary 承接，长期偏好和约束通过 Memory Writer 管理，健康/过敏等敏感内容先进 candidate；SQLite FTS/LIKE 提供基础召回，Milvus 是可选语义增强。Context Compression 没有直接依赖 LLM 自动摘要，而是先统一 PromptBuilder、结构化状态、工具 preview 和本轮 prompt compact，再用 benchmark 验证组合链路。

这套实现顺序的价值：

- 工程上：每一步都能独立测试和回滚，摘要失败也不影响完整 turn 写入。
- 面试上：能说明不是堆技术，而是在控制风险、隐私和上下文预算。
- 产品上：先解决用户能感知的连续性和个性化，再做授权、后台 worker、模型摘要和多实例治理。
