...
- 已完成：简化版持久化摘要的存储基础，包含完整 turn 事务写入、active summary 替换、消息边界和用户/会话隔离；尚未改变 Prompt。
- 已完成：确定性抽取式摘要器，包含字符阈值、摘要长度限制、上一版摘要复用和最近三轮保留；尚未接入请求链路。
...
- 下一步：统一接入 HTTP、SSE 和 WebSocket，在 Prompt 中组合 active summary 与最近原文，并在失败时继续使用现有滑动窗口。
...
- 当前最小 benchmark 已达到 8/8 passed；全量回归结果统一以[项目证据](../项目证据.md)为准，避免在实施计划中保留过期数字。
