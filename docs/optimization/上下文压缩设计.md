...
| 层级 | 当前状态 |
|---|---|
| 已实现 | `PromptBuilder` 统一 Chat、Diet、Search、MCP 主要文本 Prompt；`_structured_state` 保存任务、路由、画像、来源和工具摘要；超过阈值触发确定性 compact；记录 Prompt 长度、组成部分和压缩轨迹；会话可以从 SQLite 恢复；持久化摘要的保存、替换和消息边界已接通存储层 |
| 部分实现 | 工具结果摘要和长期记忆按预算注入；Memory + Context + RAG benchmark 已覆盖关键组合链路；确定性持久化摘要器已独立完成但尚未接入请求和 Prompt；前端只展示部分执行信息 |
