...
| 层级 | 当前状态 |
|---|---|
| 已实现 | `PromptBuilder` 统一 Chat、Diet、Search、MCP 主要文本 Prompt；`_structured_state` 保存任务、路由、画像、来源和工具摘要；超过阈值触发确定性 compact；记录 Prompt 长度、组成部分和压缩轨迹；会话可以从 SQLite 恢复 |
| 部分实现 | 工具结果摘要和长期记忆按预算注入；Memory + Context + RAG benchmark 已覆盖关键组合链路；前端只展示部分执行信息 |
| 生产化补齐 | 精确 token 计数、模型摘要与摘要持久化、完整前端状态卡片、摘要质量评测、跨实例上下文和审计治理 |

面试时应表达为：

> 当前版本已经完成“近期对话 + 结构化状态 + 长期记忆 + 确定性 compact + Prompt 元数据”的最小闭环；下一步重点是模型摘要、持久化、前端可视化和更大规模质量评测。
