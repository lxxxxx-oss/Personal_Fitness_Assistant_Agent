...
| 层级 | 当前状态 |
|---|---|
| 已实现 | `PromptBuilder` 统一主要文本 Prompt；超过阈值触发本轮 compact；会话可以从 SQLite 恢复；持久化摘要支持事务写入、active 替换、消息边界、确定性增量抽取、最近六轮保留、三种聊天协议接入和最终回答 Prompt 注入 |
...
| 触发点 | 配置 | 当前行为 |
|---|---|---|
| 持久化会话摘要 | `CONVERSATION_SUMMARY_TRIGGER_CHARS`，默认 3000 | 回答保存后，较早且未压缩消息超过阈值时，生成抽取式 active summary；始终留下 `MEMORY_MAX_TURNS` 对应的最近原文，默认 6 轮（12 条消息）不进入摘要 |
| 本轮 Prompt compact | `COMPACT_TRIGGER_CHARS`，默认 6000 | PromptBuilder 按 Markdown 标题区段保留角色/规则、结构化摘要和用户问题，保证不超过 `MAX_PROMPT_CHARS` |

“最近 6 轮原文 + 更早轮次抽取式摘要”已经统一落地。Prompt 近期窗口最多读取 `MEMORY_MAX_TURNS * 2` 条消息，持久化摘要器使用同一配置计算保护边界；默认均为 6 轮、12 条消息，不再维护容易产生分叉的独立消息数参数。摘要内容来自原始消息片段，不调用额外 LLM。
