...
| 工具系统容易被问是否只是函数调用 | 已补最小 `ToolSpec + ToolRegistry` 原型，统一管理 schema、权限、超时字段、有限重试、fallback、`execution_id`、`duration_ms` 和审计；Search、Knowledge/RAG 与 MCP execute 已接入；Motion compare 只是后续候选，当前暂缓 | 后续如继续工具治理，再评估 Motion 标准动作比较是否接入 Registry；媒体上传和姿态估计继续保留 API 边界 |
...
3. ToolRegistry 阶段已先收束，详见 [阶段总结](./progress/2026-07-09-tool-registry-stage-summary.md)；后续如继续工具治理，再评估 Motion 标准动作比较是否接入 `ToolRegistry`。
