...
```powershell
D:\Users\Lesedi\anaconda3\envs\fitness-agent\python.exe scripts/eval_rag.py --embedding-model BAAI/bge-small-zh-v1.5 --require-embedding --show-cases
```
...
如果输出的 `modes` 是 `keyword`，说明 Sentence-Transformers 模型没有成功加载。这是明确降级，不是 embedding 评测结果。当前本机默认 `text2vec-base-chinese` 缓存不完整，所以上述可复现基线显式指定了本机完整缓存的 BGE 模型；暂未因此直接修改项目默认模型。
