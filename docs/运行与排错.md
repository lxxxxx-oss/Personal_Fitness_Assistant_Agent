...
```powershell
D:\Users\Lesedi\anaconda3\envs\fitness-agent\python.exe scripts/eval_rag.py --require-embedding --show-cases
```
...
当前 15 条小规模基线显示：使用本机缓存的 `BAAI/bge-small-zh-v1.5`、Top-K=5、阈值 0.50 时，Recall@5 为 100%、MRR 为 0.892、来源命中率为 100%，但无答案拒绝率仅 33.3%。阈值提高到 0.56 后，无答案拒绝率为 100%，Recall@5 为 91.7%。这组结果用于展示阈值取舍，不代表已经完成大规模或线上效果评测。

如果输出的 `modes` 是 `keyword`，说明 Sentence-Transformers 模型没有成功加载。这是明确降级，不是 embedding 评测结果。当前默认模型已统一为 `BAAI/bge-small-zh-v1.5`，本机离线缓存可以直接复现上述基线；新机器首次运行仍需联网下载模型，或预先准备 Hugging Face 缓存。Milvus 路径会把这种情况返回为 `CONFIG_MISSING`，并附带 `embedding_model` 与 `fallback_reason`，而不是模糊的内部错误。
