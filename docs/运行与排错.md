...
```powershell
python scripts/eval_rag.py --require-embedding --show-cases
```

评测集位于 `data/eval/rag_eval.jsonl`，同时标注正确来源、正确片段关键事实和无答案问题。脚本输出 Recall@K、MRR、来源命中率、无答案拒绝率以及查询平均/P95 延迟。`--require-embedding` 用来防止模型加载失败后把关键词降级结果误记为 embedding 基线；`--min-recall`、`--min-mrr` 和 `--min-no-answer-rejection` 可用于持续回归门槛。

已记录的 15 条小规模基线来自具备本地 BGE 缓存的评测环境：使用 `BAAI/bge-small-zh-v1.5`、Top-K=5、阈值 0.50 时，Recall@5 为 100%、MRR 为 0.892、来源命中率为 100%，但无答案拒绝率仅 33.3%。阈值提高到 0.56 后，无答案拒绝率为 100%、Recall@5 为 91.7%。这组结果用于展示阈值取舍，不代表已经完成大规模或线上效果评测。

如果输出的 `modes` 是 `keyword`，说明 Sentence-Transformers 模型没有成功加载。这是明确降级，不是 embedding 评测结果；使用 `--require-embedding` 时脚本应以非零状态退出。当前默认模型是 `BAAI/bge-small-zh-v1.5`，但新机器仍需联网下载或预先准备 Hugging Face 缓存。Milvus 路径会把模型不可用返回为 `CONFIG_MISSING`，并附带 `embedding_model` 与 `fallback_reason`，而不是模糊的内部错误。
