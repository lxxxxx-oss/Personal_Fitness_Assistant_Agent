...
> 复盘后发现菜谱与 Diet 的职责高度重叠，而且当前 MCP 路径主要依赖 mock，对健身主线贡献有限，却增加了协议、路由和测试成本。为了让项目更聚焦、也让面试表达与自己的掌握程度一致，我决定把菜谱并入 Diet，并在后续代码迭代中删除 MCP 路由。

这是一个架构收敛决定，不代表 MCP 技术没有价值，只代表它不是这个项目当前最值得保留的能力。

### Q14：代码还没删除，面试时怎么解释？

> 当前仓库仍保留早期 MCP Client、`mcp` intent 和相关测试，因此不能说代码已经完成四路由迁移。准确说法是：目标架构已经收敛为 Chat、Search、Diet、Motion 四个模块，MCP 是待清理的遗留实验；代码删除后会重新运行 Router eval 和 pytest，更新新的基线数字。
...
> 当前代码基线是 pytest 收集到的 114 个自动化用例，覆盖 API、Router、eval 脚本、Retriever、LLM Loader 缓存、Motion、PoseSequence、姿态适配器、搜索工具和记忆，也包含尚未删除的 MCP 遗留测试。为了离线、快速、可重复执行，测试全局 mock 了 LLM 和 SentenceTransformer，外部服务和真实 MediaPipe 也主要通过 mock 或降级路径验证。
...
> 当前是本地面试原型，没有认证、权限和限流，CORS 也较宽松。生产化至少要补：用户认证、会话隔离、速率限制、上传文件大小和 MIME 校验、Prompt Injection 防护、搜索内容隔离、敏感日志脱敏和审计。
...
> LangGraph、MediaPipe、Tavily 和 Qwen 是基础能力。我亲自完成的是它们之间的控制面和工程胶水：Router 规则与元数据、eval 数据和脚本、状态和组合执行、错误隔离、PoseSequence 适配层、接口与 UI 串联、共享模型加载和测试体系。我的创新不在于重新训练基础模型，而在于受控集成和可验证演进。
...
> 当前最薄弱的是端到端质量证据：Router 有回归集，但 RAG、真实 LLM 回答和 Motion 标准动作判断还缺冻结数据集与真实环境验收。下一步应该先删除重叠的 MCP 菜谱链路，再把精力放在数据和评测上，而不是继续增加模块。

### Q27：如果只允许你生产化一个模块，先做哪个？

> 我会先把 Router、RAG 和 Chat 主链路生产化，因为它们有最高使用频率，也最容易建立真实数据闭环。先补认证、日志、引用、RAG eval、模型服务和会话持久化；Motion 保持受控实验入口，等数据和业务价值明确后再扩大。
...
- MCP 是一次已经完成的技术实验，但因为业务重叠和面试主线不聚焦，决定从核心架构中移除。
