...
| ToolRegistry | 工具元数据和执行治理中心。 | 已落地最小原型，并让 Search 与 Knowledge/RAG 通过 Registry 调用 `search.tavily` 和 `knowledge.retrieve`。 | `ToolSpec(name, description, input_schema, permission, executor, timeout, retry, fallback)` 加 `register/list/validate/execute`；结果和 audit log 记录 `execution_id`、`duration_ms`、attempts、fallback 归因；默认注册四类代表工具。 | LangChain Tools、OpenAI Function Calling、MCP tools/list、自定义 if/else 调用。 | 当前不替代 LangGraph，也不让 LLM 自由发现和调用工具；Search/Knowledge 已接入，Motion/MCP 因文件、数组和子进程边界更复杂，保持谨慎迁移。 |
