...
| ToolRegistry | 工具元数据和执行治理中心。 | 已落地最小原型，并让 Search 子图作为第一个真实路径通过 Registry 调用 `search.tavily`。 | `ToolSpec(name, description, input_schema, permission, executor, timeout, retry, fallback)` 加 `register/list/validate/execute`；默认注册 `knowledge.retrieve`、`search.tavily`、`motion.compare_pose`、`mcp.call_tool` 四类代表工具。 | LangChain Tools、OpenAI Function Calling、MCP tools/list、自定义 if/else 调用。 | 当前不替代 LangGraph，也不让 LLM 自由发现和调用工具；Search 已接入，Motion/MCP 因文件、数组和子进程边界更复杂，保持谨慎迁移。 |
