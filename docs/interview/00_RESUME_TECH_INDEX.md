...
| ToolRegistry | 工具元数据和执行治理中心。 | 已落地最小原型，并让 Search、Knowledge/RAG、MCP execute 通过 Registry 调用 `search.tavily`、`knowledge.retrieve` 和 `mcp.call_tool`。 | `ToolSpec(name, description, input_schema, permission, executor, timeout, retry, fallback)` 加 `register/list/validate/execute`；结果和 audit log 记录 `execution_id`、`duration_ms`、attempts、fallback 归因；默认注册四类代表工具。 | LangChain Tools、OpenAI Function Calling、MCP tools/list、自定义 if/else 调用。 | 当前不替代 LangGraph，也不让 LLM 自由发现和调用工具；Motion 因文件、数组和媒体 API 边界更复杂，保持谨慎迁移。 |
